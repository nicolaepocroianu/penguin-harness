...
export const en: Strings = {
  ...
  activities: {
    ...
    workspaceRail: "Activity",
    studioTree: {
      label: "Activity hierarchy",
      rows: {
        activityScript: "Activity Script",
        activitySpec: "Activity Spec",
        implementationFeatures: "Implementation Features",
    ...
    studioPlayer: {
      ...
      map: {
        ...
        inspect: {
          hint: "Choose a phase to see what it does. Hold Ctrl and scroll to zoom.",
          title: (phase: string) => `Phase ${phase}`,
          initial: "First phase",
          final: "Final phase",
          entry: "On entry",
          exit: "On exit",
          invokes: "Runs",
          outgoing: "Moves on",
          incoming: "Reached from",
          via: (trigger: string) => `on ${trigger}`,
          to: "to",
          leaves: (trigger: string, target: string) => `on ${trigger}, leaves for ${target}`,
          nothing: "Nothing",
          close: "Close",
        },
      },
    ...
    studioRefs: {
      ...
      stable: "Stable",
      stableHint:
        "A stable ref is one other refs may be built from. Mark it stable once its content is settled.",
    ...
    studioBoard: {
      ...
      previous: "Previous",
      next: "Next",
      previousScene: (scene: string) => `Previous scene: ${scene}`,
      nextScene: (scene: string) => `Next scene: ${scene}`,
      open: "Open",
      scene: (n: number, id: string) => `Scene ${n}, ${id}`,
      shared: "Shared media",
      working: "An agent is working on this scene",
      proposed: "A proposal waits for this scene",
      unbound: (n: number) => `${n} without media`,
      assets: (n: number) => `${n} ${n === 1 ? "asset" : "assets"}`,
    ...
    studioRun: {
      ...
      run: "Run",
      stop: "Stop",
      idle: "Run every stage, from the script to an assembled module, or pick one stage to run again. Each stage is an ordinary run in Generation History.",
      progress: (done: number, total: number) => `${done} of ${total}`,
      notes: {
        planCurrent: "The media plan already matches the specification.",
        allTranslated: "Every narration is translated.",
      ...
      stopped: "Stopped. Nothing after the stopped stage ran.",
      failed: (reason: string) => `Stopped at a failure: ${reason}`,
      saveFirst: "Save your edits before running stages.",
    ...
    studioScript: {
      ...
      scenes: "Scenes",
      scenesHelp: "Fold every scene to its heading",
      diff: "Diff",
      diffOff: "Off",
      diffSaved: "Since last save",
      diffProposal: "Agent proposal",
      stats: (added: number, removed: number) => `+${added} −${removed}`,
      noChanges: "No changes",
      changedScenes: "Changed scenes",
      scene: (n: number) => `Scene ${n}`,
      proposed: "proposed, not applied",
      proposalShown: "Agent proposal, read-only",
      acceptProposal: "Accept the proposed script",
      save: "Save script",
      ...
      editorPhrases: {
        ...
        Accept: "Accept",
        Reject: "Reject",
        "Expand $ unchanged lines": "Expand $ unchanged lines",
      },
      autosave: {
    ...
    studioConversation: {
      ...
      asset: (key: string, scene: string | null) => (scene ? `${key} in ${scene}` : key),
      scene: (scene: string) => `scene ${scene}`,
      placeholder: "Ask the agent about this activity",
      send: "Send",
      stop: "Stop",
      ...
      noAgent: "Choose an agent first.",
      movedTo: (focus: string) => `(I am now looking at ${focus}.)`,
    },
    railCollapse: "Collapse the activity rail",
    railExpand: "Expand the activity rail",
    railWidth: "Activity rail width",
    sectionNames: {
    ...
    activityStats: {
      reload: "Reload",
      unreadable: (reason: string) => `The statistics could not be read: ${reason}`,
      empty: "The media plan asks for no audio, video, image or animation.",
      type: "Asset type",
      language: "Language",
      count: "Assets",
      bound: "Bound",
    ...
    waveformTrim: {
      ...
      noAudio: "this browser cannot decode audio",
      remains: (length: string) => `${length} remains`,
      playRemaining: "Play remaining",
      marked: (at: string) => `Start marked at ${at}. Press Enter again to end the selection.`,
      keys: "Arrow keys move the playhead, a tenth of a second at a time or a second with Shift. Home and End go to either end. Enter marks the start, then the end, of a selection; Escape clears it.",
      fetchFailed: (status: number) => `the clip could not be fetched (${status})`,
    },
  ...
  models: {
    ...
    cliEnvOverwriteBody: (key: string) => `${key} already has a value. Saving replaces it.`,
    cliEnvRemoveTitle: "Remove this variable?",
    cliEnvRemoveBody: (key: string) => `The agent starts without ${key} from its next start.`,
    localCliIntro:
      "Choose which coding agent runs your prompts. Each one runs on the server machine and signs in on its own, so it needs no API key here.",
    installedClis: (n: number) => `Installed CLIs (${n})`,
    availableClis: (n: number) => `Available CLIs (${n})`,
    cliRescan: "Rescan",
    cliScanning: "Scanning…",
    cliAddAgent: "Add agent",
    cliModel: "Model",
    cliEffort: "Reasoning effort",
    cliSynced: "Synced from CLI",
    cliDefault: "CLI default",
    cliModelsUnknown: "Rescan to read the models this CLI offers.",
    cliSignedIn: "Signed in",
    cliSignInRequired: "Sign-in required",
